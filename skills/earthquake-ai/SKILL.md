...
## Description
地震學 AI 助手，整合地震資料查詢、PyGMT 繪圖、Gemini AI 解釋、NotebookLM 教材生成等功能。專門用於地震學教學與研究支援。

## Features
- 📡 從 USGS/TAIGER 即時取得地震資料
- 🗺️ 用 PyGMT 繪製地震分布圖、截面圖、走時曲線
- 🤖 Gemini AI 解釋地震現象與即時資料分析
- 📚 自動生成地震學教材與筆記 (NotebookLM 風格)
- 🌐 一鍵部署 Gradio Web 應用到 HuggingFace Space
- 📊 地震目錄統計與視覺化 (pandas + matplotlib)
- 🧭 快速報告摘要與課堂用圖模板
...
### 3. gemini.explain
使用 Gemini AI 解釋地震現象或分析資料。

**參數：**
```json
{
  "question": "學生的提問，例如：'什麼是走時曲線？'",
  "context": "相關資料或圖表描述（可選）"
}
```
...
# 4. 課堂問答
openclaw agent --message "學生問：'P 波和 S 波有什麼不同？' 用簡單方式回答"
```

## 教學流程建議

1. **引導問題**：先讓學生提出對地震的疑問，再由 `gemini.explain` 統整概念。
2. **資料探索**：用 `usgs.fetch` 取得事件，搭配 `pygmt.plot` 繪製可視化圖表。
3. **統計分析**：透過 `stats.analyze` 讓學生理解規模與深度分布。
4. **教材輸出**：使用 `notebooklm.generate` 產生講義與小測驗。
...
- 建議在圖表或教材中標註資料來源與更新時間。
- USGS/TAIGER 可能存在震級修正，課堂中可提醒學生注意回溯更新。
- API 金鑰請使用環境變數或 Secrets 管理，避免公開。
